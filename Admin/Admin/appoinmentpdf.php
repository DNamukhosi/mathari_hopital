...
<button><a style = "text-decoration:none; color:#000;" href = "today.php">Back</a></button>
<?php
    $dt = date("Y-m-d");
    $conn = new mysqli("localhost", "root", "", "rukangadms") or die(mysqli_error());
    $query = $conn->query("SELECT appointment.pat_no,appointment.patient,patient.phone,patient.gender,patient.address FROM `appointment` left join patient on patient.id = appointment.pat_no where app_date= '$dt'") or die(mysqli_error());
    $f = $query->fetch_array();
?>
...
<br />
    <div id="print">
        <div style = "margin:10px;">    <br />      
            ...
            <center>Rukanga Health Center</center>
            <br />
             <center><b>Todays List</b></center>
            <br />
            <label>Date:
                <?php echo "<u>".
                       date("d-m-Y")."</u>";
            ...
            <center>
                <table>
                        <tr>
                            ...
                            <th style = "padding-left:30px; padding-right:30px;"><center>Phone</center></th>
                        </tr>
                <?php
                $query =$conn->query( "SELECT appointment.pat_no,appointment.patient,patient.phone,patient.gender,patient.address FROM `appointment` left join patient on patient.id = appointment.pat_no where app_date= '$dt'") or die(mysqli_error());
        ...
        $cnt=$query->num_rows;
                ...
                ?>
                    <tr>
                        ...
                        <td><center><?php echo $fetch['patient']?></center></td>
