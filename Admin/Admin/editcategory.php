...
<?php 
  include("../../inc/connect.php") ;
    ...
    //session_start();
  $sql="SELECT * FROM medicinalcategory WHERE id='".$_GET['id']."'";
...
{ //echo $write; exit();
    ...
    $write=mysql_query("UPDATE medicinalcategory SET category='$category',description='$description' WHERE id='".$_GET['id']."'") or die(mysql_error($db_connect));
   //print_r($write); exit();
      //$query=mysql_query("SELECT * FROM user ")or die (mysql_error());
      //$numrows=mysql_num_rows($query)or die (mysql_error());
      echo " <script>setTimeout(\"location.href='medicalcategory.php';\",150);</script>";
}
...
<?php
 ...
 $p_query="SELECT * FROM medicinalcategory";
