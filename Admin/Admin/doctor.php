...
$dno=str_pad($lno, intval($ff['length']), "0", STR_PAD_LEFT);
   ...
   if( $password==$cpassword){
    ...
    $nrows=$qry->fetch_all();
     if($nrows==null)
      {
        $write =$conn->query("INSERT INTO sys_user(`user_id`,`username`,`password`,`level`) VALUES ('$dno','$username','$password','$id')") or die(mysqli_error());

            $writex =$conn->query("INSERT INTO doctors(`d_id`,`firstname`,`middlename`,`lastname`,`username`,`phone`,`idno`,`password`) VALUES ('$dno','$fname','$mname','$lname','$username','$phone','$idno','$password')") or die(mysqli_error());

            if($write && $writex)
             {  echo " <script>alert('Doctor Records Successfully Inserted..');</script>";
  ...
  <div class="col-xs-12">
    <div class="box box-primary">
      <div class="box-header with-border ">
        <i class="fa fa-user"></i> <h3 class="box-title">DOCTORSS</h3>
      </div>
...
$query=$conn->query("SELECT d_id,concat(firstname,' ',middlename,' ',lastname) as name,username,phone,idno FROM doctors order by d_id")or die (mysqli_error());
...
<td><a href="editdoctor.php?id=<?php echo $row['d_id']; ?>"><span class="btn btn-success bg-green"><i class="fa fa-edit"></i> Edit </span></a>
 <a href="delete.php?id=<?php echo $row['d_id']; ?>"><span class="btn btn-danger"><i class="fa fa-trash-o"></i> Delete </span></a></td>
</tr>
<?php }
$conn->close();
  ?>
